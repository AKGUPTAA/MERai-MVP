...
Initially, I considered building a generic data visualization tool, drawing on standard BI dashboard patterns. However, I quickly realized that a broad, generic tool wouldn't land with the Minto.ai founder. I needed a vertical SaaS product that solved a specific, expensive problem. EPC project closeouts are notoriously chaotic and cost companies millions in delayed handovers—this felt like the perfect wedge.
...
- **The Stack:** I chose React + Vite + Tailwind CSS instead of Next.js or Streamlit. Why? Speed. Next.js introduces unnecessary server-side overhead for a pure prototype. Vite provides instant HMR, and Tailwind allowed me to hit the "premium dark luxury SaaS" aesthetic rapidly without writing heavy custom CSS.

## 3. Reversals & Changes
- **Backend Architecture:** I initially planned to wire up a real Python backend with an LLM API. I reversed this decision midway. For a 48-hour prototype demo, latency and prompt tuning are risks. Hardcoding a highly specific, realistic scenario (the Paradip Green Hydrogen Facility) demonstrates the value proposition much more effectively than a generic RAG pipeline running on random text.
...
- The Tailwind error was caught immediately upon spinning up the dev server and viewing the broken localhost UI.
...
- **Real RAG Pipeline:** Integrating LangChain/LlamaIndex to genuinely parse unstructured EPC dossiers.
