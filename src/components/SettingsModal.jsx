    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2"><Key className="w-5 h-5 text-blue-600"/> API Settings</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-5 h-5"/></button>
        </div>
        <div className="p-6">
          <p className="text-sm text-slate-500 mb-4">
            Enter your OpenAI API Key to enable real intelligent document parsing and chat. Your key is stored safely in your local browser storage.
          </p>
          <input 
            type="password"
            value={inputKey}
        ...
        <div className="bg-slate-50 p-6 flex justify-end gap-3 border-t border-slate-100">
          ...
          <button onClick={() => setApiKey(inputKey)} className="px-6 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors shadow-sm">Save Key</button>
        </div>
      </div>
    </div>
  );
